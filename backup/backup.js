...
// -------- SMART AUDIO PLAYBACK WITH SPOTIFY PREVIEW FALLBACK --------
async function findAndPlayAudio(connection, song) {
    ...
    // Debug: Check if preview URL exists
    ...
    if (song.preview_url) {
        try {
            ...
            console.log(`Spotify preview failed: ${error.message}, trying YouTube...`);
        }
    } else {
        ...
        try {
            ...
            const searchResult = await searchSpotifyForPreview(song.title, song.artist);
...
async function searchSpotifyForPreview(title, artist) {
    try {
    ...
    return new Promise((resolve, reject) => {
        ...
        const request = https.get(previewUrl, (response) => {
            ...
            try {
                ...
                console.log(`✅ Playing Spotify preview: ${title} by ${artist}`);
                resolve();
            } catch (err) {
                reject(err);
            }
...
async function playFromYouTube(player, songTitle, songArtist) {
    return new Promise((resolve, reject) => {
        ...
        ytDlpProcess.stdout.on('data', (chunk) => {
            ...
            if (!hasStarted && bytesReceived > 64 * 1024) {
                ...
                try {
                    ...
                    player.play(resource);
                    console.log(`✅ Playing from YouTube: ${songTitle} by ${songArtist} (${Math.round(bytesReceived / 1024)}KB)`);
                    resolve();
                } catch (err) {
        ...
        ytDlpProcess.stdout.on('error', (err) => {
            if (err.code === 'EOF' || err.message.includes('EOF')) {
                console.log('Stream ended (EOF) - normal');
                return;
            }
            if (!hasStarted) {
                cleanup();
        ...
        timeout = setTimeout(() => {
            if (!hasStarted) {
                cleanup();
                reject(new Error('YouTube audio timeout'));
            }
        }, 20000);
    });
}
    ...
    try {
        ...
        while (offset < 1000) { // Limit to 1000 tracks max
            try {
                ...
                // Debug: Count tracks with previews
        ...
        // Final summary
        const totalWithPreviews = allTracks.filter(t => t.preview_url).length;
        console.log(`📊 Playlist Summary: ${allTracks.length} total tracks, ${totalWithPreviews} (${Math.round(totalWithPreviews / allTracks.length * 100)}%) with Spotify previews`);
...
async function playRoundAudio(voiceChannel) {
    ...
    if (!connection || connection.state.status === VoiceConnectionStatus.Destroyed || connection.state.status === VoiceConnectionStatus.Disconnected) {
        ...
        connection.on(VoiceConnectionStatus.Disconnected, async (oldState, newState) => {
            try {
                ...
                console.log('Connection lost, will reconnect on next round');
    ...
    try {
        ...
        // Try smart audio playback (Spotify preview first, then YouTube)
        let player;
        try {
            player = await findAndPlayAudio(connection, gameState.currentSong);
        } catch (primaryError) {
            ...
            player = await findAndPlayAudio(connection, simplifiedSong);
        }
